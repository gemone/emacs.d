...
#+NAME: pip-install
#+begin_src emacs-lisp :tangle no :var package=""
  (concat "pip3 install -U " package)
#+end_src

=shell-interpreter=, use =pyvenv= to handle

#+begin_src emacs-lisp :noweb yes
  (setenv "DEFAULT_PYTHON_ENV" (expand-file-name "<<python-venv()>>" user-emacs-directory))
...
#+begin_src emacs-lisp :tangle no :var package=""
  (concat "[[ -e `which npm` ]] && npm install --prefix ./etc/npm " package)
#+end_src

#+NAME: pwsh-npm-install
#+begin_src emacs-lisp :tangle no :var package=""
  (concat "if (Test-Path (where.exe npm)) {
    npm install --prefix ./etc/npm "  package
    "
  }")
#+end_src
...
** [[https://github.com/kaiwk/leetcode.el][leetcode.el]]

You can add =(setq enable-leetcode t)= in =init.local.el= to add =leetcode.el= for you emacs.

You will find leetcode-cli config in =~/.leetcode/leetcode.toml=.

please install [[https://github.com/clearloop/leetcode-cli][=leetcode-cli=]]
